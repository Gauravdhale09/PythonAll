...
# root.wm_iconbitmap('e:/MAIN/Python Learn/Room_book/49616_bill_invoice_method_payment_sales_icon.ico')
root.wm_iconbitmap('E:\MAIN\Python Learn\Room_book\invoice.ico')

root.title('E-bill/Register')
font = f.Font(family="Comic Sans MS", size=25, weight="bold")
photo = Image.open('E:\MAIN\Python Learn\Room_book\door.png')
p = photo.resize((1500, 996))
photo_r = ImageTk.PhotoImage(p)
Label(root, image=photo_r).place(x=0, y=0, relheight=1, relwidth=1)
img = Image.open('E:\\MAIN\\Python Learn\\Room_book\\signup.png')
...
photo_h = PhotoImage(file='e:/MAIN/Python Learn/Room_book/hide.png')
photo_s = PhotoImage(file='e:/MAIN/Python Learn/Room_book/show.png')
c1 = Button(frame1, image=photo_s, command=show,
            bd=0, relief=SUNKEN).place(x=1200, y=471)
e5_var = StringVar()
E5 = Entry(frame1, font=('Roboto', 15), width=30,
           textvariable=e5_var, relief=SOLID).place(x=950, y=540)
photo_si = PhotoImage(file='e:/MAIN/Python Learn/Room_book/signup1.png')
b1 = Button(frame1, image=photo_si, cursor='hand2', relief=FLAT,
            bd=0, command=ok).place(x=700, y=590, width=200, height=50)
l2 = Button(frame1, text="*Generate password", fg='blue', font=('Roboto', 22, 'italic'), bd=0, background='#afd8db',
            activebackground='#afd8db', activeforeground='blue', cursor='hand2', width=30, relief=FLAT, command=gen).place(y=580, x=900)
photo_log = PhotoImage(file='e:/MAIN/Python Learn/Room_book/log.png')
photo_re = PhotoImage(file='e:/MAIN/Python Learn/Room_book/register.png')
photo_up = PhotoImage(file='e:/MAIN/Python Learn/Room_book/login1.png')
photo_in = PhotoImage(file='e:/MAIN/Python Learn/Room_book/info.png')


def info():
    info = Tk()
    info.geometry('500x100')
    info.wm_iconbitmap('e:/MAIN/Python Learn/Room_book/invoice.ico')
    info.config(background='yellow')
    info.title('E-bill/help')
    info.maxsize(500, 100)
    info.minsize(500, 100)
